...
def constrained_kmeans_binary(X, 
    ...
    while (not converged and iter < max_iter):       
        ...
        #pdb.set_trace()                               
        #S = assignment_matrix_from_labels(labels)  
        obj = distortion(X, C, S)
...
def binary_assignment(X, 
    ...
    with mosek.Env() as env:     
        with env.Task(0,0) as task:                 
            ...
            it = 0    
            alpha = 0.5
            scale_factor = 1# np.sqrt(nClusters*N)/np.linalg.norm(XC)
            ...
            while not converged and it < maxIter:                           
                ...
                for j in range(N):
                    for i in range(nClusters):
                        #print rho_up[i,j], rho_lo[i,j]
                        
                        # if is_binary(St[i,j])<0:
                        #     print i,j,St[i,j] 
                        #     print rho_plus[i,j], rho_minus[i,j]                               
